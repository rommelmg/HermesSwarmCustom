...
class PromptComplexityClassifier:
    ...
    def __init__(self):
        ...
        # (raw_pattern, weight)
        # Negative weights reduce the score for simple/trivial prompts.
        _pattern_groups: dict[str, list[tuple[str, int]]] = {

            # --- Simple lookups: push score down ---
            "simple": [
            ...
            "complex_task": [
                (r"\bdiseñ",                                                2),  # diseña, diseño, diseñar, design
                (r"\bdesign\b",                                             2),
                (r"\barquitectura\b|\barchitecture\b",                      2),
                (r"\bestrategia\b|\bstrateg",                               2),  # estrategia, strategy, strategic
                (r"\boptimiz",                                              2),  # optimiza, optimización, optimize, optimization
                (r"\bcompar",                                               2),  # compara, comparación, compare, comparison
                (r"\bevalú|\beval[uú]",                                     2),  # evalúa, evaluate
                (r"\bassess\b",                                             2),
                (r"\btrade[- ]?off\b",                                      2),
                (r"\bimplementa\b|\bimplement\b|\bconstruye\b",            2),
                (r"\bimplementación\b|\bimplementation\b",                  1),
                (r"\brefactor",                                             2),  # refactor, refactoriza, refactoring
                (r"\breestructura\b",                                       2),
            ...
            # --- Mathematical / scientific complexity ---
            "math_science": [
            ...
            "code_technical": [
                ...
                (r"\bdistributed\b|\bdistribuido\b|\bconcurren",            2),  # concurrent, concurrente, concurrencia
                ...
                (r"\bperformance\b|\brendimiento\b|\blatenc",               1),  # latency, latencia
                (r"\bci[/\-]?cd\b|\bdevops\b|\bdocker\b|\bkubernetes\b|\bk8s\b",         1),
                (r"\bsql\b|\bnosql\b|\borm\b|\bmigraci",                   1),  # migración, migration
                (r"\bunit test\b|\btest unitario\b|\bintegration test\b",   1),
                (r"\bautenticaci|\bauthenticat",                            1),  # autenticación, authentication
            ],
            ...
            # --- Hypothetical / conditional reasoning ---
            "hypothetical": [
    ...
    def score(self, prompt: str) -> tuple[int, dict]:
        ...
        # 2. Pattern groups (simple group can produce negative contributions)
        ...
        # 4. Comma density (proxy for multi-condition complexity)
        commas = prompt.count(",")
        if commas > 7:
            score += 2
